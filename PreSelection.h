...
// Thu Jun  9 17:53:17 2022 by ROOT version 6.22/06
// from TTree PreSelection/PreSelection
// found on file: newSample1.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
  ...
  TFile* tf_out;		       // TFile for output root file
  void BookHistograms(TString ofroot); // Open output root file and define the histograms, graphs to plot
  vector<int>pid_list;

  // List of histgrams, graphs
  TH1F* hGenNum[15];	// Gen particle multiplicity
  TH1F* hGenMass[7];	// Gen particle mass  
  TH1F* BosDecay[2];		// pdgID of Daughters of W,Z,H
  TH1F* BosPt[2];		// W/Z Pt

  // With and without Lepton veto
  TH1F* hGenMET[2];
  TH1F* hGenJetPt[2][2];		 // leading, subleading jet Pt
  TH1F* hGenJetMass[2][2];		 // leading, subleading jet Mass
  TH1F* hNGenJets[2][2];		 // No. of AK8,AK4 jets
  TH1F* hGenHT[2];			 // GenHT

  // Finding out the source of high MET
  TH1F* hGvPt[2];			 // Total Pt of gravitinos
  TH1F* hUKMET[2];			 // unknown MET = MET-Total GVino Pt
  TH1F* hUKMET_Vec[2];			 // unknown MET = MET-Total GVino Pt (vectorial subtraction)
  TH1F* hTotPt[2];			 // Total Pt of both chi's
  TH1F* hMET_Gv_dPhi[2];		 // dPhi between gvino and MET
  TH1F* hJetPt[2][2];		 // leading, subleading reco jet Pt
  TH1F* hJetMass[2][2];		 // leading, subleading reco jet Mass

  // Events with 3 Gen AK8 jets
  TH1F* hBosPt_3AK8[2];		// W,Z Pt
  TH1F* hSgetPt_3AK8[2];	// Leading, subleading singlet Pt.
  TH1F* hJetPt_3AK8[2][2];	// leading, subleading reco jet Pt
  TH1F* hJetMass_3AK8[2][2];	// leading, subleading reco jet Mass
  
  // Object to handle PDGIDs and particle names
  MyPDGParticles* myPDG;

  // User defined functions
  
  // 1. takes a vector of TLorentzVectors, sorts them w.r.t. Pt in descending order and returns it
  vector<TLorentzVector> SortPt(vector<TLorentzVector>*);

  // 2. Construct images of histograms. outkey = string which will be attached to the pdf file names
  void MakePDF(TString outkey);

  // Flags for various cuts
  bool LeptonVeto{false};		// true-> passes lepton veto
  bool GenAK8Jet3{false};		// true-> only 3 Gen AK8 jets in the event
};
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ROOT_IN/newSample1.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ROOT_IN/newSample1.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("ROOT_IN/newSample1.root:/TreeMaker2");
      dir->GetObject("PreSelection",tree);

   }
   Init(tree);
   myPDG = GetParticles();		// Get MyPDGParticles object
}
...
Int_t PreSelection::Cut(Long64_t entry)
{
  ...
  for(int pp=0;pp<GenParticles_PdgId->size();pp++){ // Loop over gen particles
    int pid = abs(GenParticles_PdgId->at(pp));
    if(10<pid && pid<17){
      LeptonVeto=false;
      break;
    }
